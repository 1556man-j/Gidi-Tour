<script lang="ts">
  ...
  type Story = { title: string; tag: string; image: string; href?: string };

  let { stories }: { stories?: Story[] } = $props();

  const fallbackStories: Story[] = [
    {
      title: 'How to experience Lagos like a local',
      tag: 'Lagos',
      image: 'https://images.unsplash.com/photo-1618828665011-0abd973f7bb8?auto=format&fit=crop&w=1000&q=80',
    },
    {
      title: 'A first-timer\u2019s guide to Ghana beyond Accra',
      tag: 'Ghana',
      image: 'https://images.unsplash.com/photo-1576485290814-1c72aa4bbb8e?auto=format&fit=crop&w=1000&q=80',
    },
    {
  ...
  const storyList = $derived(stories && stories.length ? stories : fallbackStories);
  ...
  <div class="mx-auto max-w-[1440px]">
    <div class="reveal grid gap-8 lg:grid-cols-12 lg:items-end">
      ...
      <div class="flex items-center justify-between gap-4 lg:col-span-4 lg:col-start-9 lg:justify-end">
        <p class="max-w-md text-sm leading-6 text-black/55">
          Real trips, real people. Dispatches, guides and lessons from the road, written by the ones who went.
        </p>
      </div>
    </div>
    ...
    <div class="mt-8 flex items-center gap-4">
      ...
      <div class="story__track" bind:this={storyTrack} onscroll={updateStoryActiveDot}>
        {#each storyList as story (story.title)}
          <a
            href={story.href ?? '/stories'}
          ...
          >
            ...
            <div class="absolute inset-x-0 bottom-0 p-5">
              <p class="mb-2 text-[10px] font-bold uppercase tracking-[.18em] text-[#F98315]">{story.tag}</p>
    ...
    <div class="flex items-center justify-center gap-2 mt-8" role="tablist" aria-label="Choose story">
      {#each storyList as story, i (story.title)}
        <button
          type="button"
          class="story__dot"
