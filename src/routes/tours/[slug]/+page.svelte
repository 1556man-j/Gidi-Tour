<script lang="ts">
	...
	const related = $derived(
		tour
			? tours.filter((t) => t.slug !== tour.slug && t.countrySlug === tour.countrySlug).slice(0, 3)
			: []
	);

	let inTourList = $derived(tourStore.has(tour?.slug ?? ''));
    ...
    function handleBookNow(): void {
		...
		if (tourStore.has(tour.slug)) {
			tourStore.updateTravelers(tour.slug, travelers);
		} else {
			tourStore.add(item);
		}
		goto('/book');
...
{#if tour}
	...
	<section class="bg-[#f7f3ea] px-5 py-16 sm:px-8 lg:px-12 lg:py-24">
		<div class="mx-auto grid max-w-360 gap-10 lg:grid-cols-[1fr_380px]">
			...
			<div>
				<div
					class="rounded-[28px] border border-black/10 bg-white p-6 shadow-sm lg:sticky lg:top-28"
				>
